...
import static com.ghroth251.JcqWebService.WebServiceStart;
import static com.ghroth251.JcqWebService.linkState;
import static com.ghroth251.MapUtils.mapSave;
import static com.ghroth251.SocketUtils.sendServiceMsg;
 ...
 * 例：appid(com.example.demo) 则加载类 com.example.com.ghroth251.Socket<br>
 ...
 * 具体功能可以查看文档
 */
public class Socket extends JcqAppAbstract implements ICQVer, IMsg, IRequest {
    public static  FateServiceService service;
    public static  FateService server;
    /**
    ...
    public static void main(String[] args) {
        ...
        CQ.logInfo("[JCQ] TEST com.ghroth251.Socket", "测试启动");// 现在就可以用CQ变量来执行任何想要的操作了
        // 要测试主类就先实例化一个主类对象
        Socket socket = new Socket();
        // 下面对主类进行各方法测试,按照JCQ运行过程，模拟实际情况
        socket.startup();// 程序运行开始 调用应用初始化方法
        socket.enable();// 程序初始化完成后，启用应用，让应用正常工作
        // 开始模拟发送消息
        // 模拟私聊消息
        // 开始模拟QQ用户发送消息，以下QQ全部编造，请勿添加
//        socket.privateMsg(0, 10001, 2234567819L, "小姐姐约吗", 0);
//        socket.privateMsg(0, 10002, 2222222224L, "喵呜喵呜喵呜", 0);
//        socket.privateMsg(0, 10003, 2111111334L, "可以给我你的微信吗", 0);
//        socket.privateMsg(0, 10004, 3111111114L, "今天天气真好", 0);
//        socket.privateMsg(0, 10005, 3333333334L, "你好坏，都不理我QAQ", 0);
//        // 模拟群聊消息
//        // 开始模拟群聊消息
//        socket.groupMsg(0, 10006, 3456789012L, 3333333334L, "", "菜单", 0);
//        socket.groupMsg(0, 10008, 3456789012L, 11111111114L, "", "小喵呢，出来玩玩呀", 0);
//        socket.groupMsg(0, 10009, 427984429L, 3333333334L, "", "[CQ:at,qq=2222222224] 来一起玩游戏，开车开车", 0);
//        socket.groupMsg(0, 10010, 427984429L, 3333333334L, "", "好久不见啦 [CQ:at,qq=11111111114]", 0);
        socket.groupMsg(0, 10011, 650903307L, 553859318L, "", "atk.1", 0);
        // ......
        // 依次类推，可以根据实际情况修改参数，和方法测试效果
        // 以下是收尾触发函数
        // socket.disable();// 实际过程中程序结束不会触发disable，只有用户关闭了此插件才会触发
//        while(true);
        socket.exit();// 最后程序运行结束，调用exit方法
    }
    ...
    public String appInfo() {
        ...
        String AppID = "com.ghroth251.socket";// 记住编译后的文件和json也要使用appid做文件名
        /**
    ...
    public int startup() {
        ...
        CQ.logDebug("目录",appDirectory);
        WebServiceStart();
        CQ.logDebug("启动消息","启动完毕");
...
//            e.printStackTrace();
//        }
        QQInfo q = CQ.getStrangerInfo(fromQQ);
        String reMsg;
        LinkedHashMap<String,Object> msgMap = new LinkedHashMap<>();
        String requstMsg;
        msgMap.put("Act","QQ");
        msgMap.put("fromQQ",fromQQ);
        msgMap.put("cardName",q.getNick());
        msgMap.put("msg",msg);
        requstMsg = mapSave(msgMap);
        reMsg = linkState?sendServiceMsg(requstMsg):null;
        if(reMsg!=null){
    ...
    public int groupMsg(int subType, int msgId, long fromGroup, long fromQQ, String fromAnonymous, String msg,
        ...
        if (fromQQ == 80000000L && !fromAnonymous.equals("")) {
            ...
            Member m = CQ.getGroupMemberInfoV2(fromGroup,fromQQ);
            LinkedHashMap<String,Object> msgMap = new LinkedHashMap<>();
            String requstMsg;
            msgMap.put("Act","group");
            msgMap.put("fromQQ",fromQQ);
            msgMap.put("fromGroup",fromGroup);
            msgMap.put("cardName",m.getCard());
            msgMap.put("msg",msg);
            requstMsg = mapSave(msgMap);
            reMsg = linkState?sendServiceMsg(requstMsg):null;
        }
        ...
        // 这里处理消息
//        System.out.println(CC.at(fromQQ));
        if(reMsg!=null){
            System.out.println(reMsg);
            CQ.sendGroupMsg(fromGroup,reMsg);
        }
        return MSG_IGNORE;
    }
    ...
    public int discussMsg(int subtype, int msgId, long fromDiscuss, long fromQQ, String msg, int font) {
        ...
        LinkedHashMap<String,Object> msgMap = new LinkedHashMap<>();
        String requstMsg;
        msgMap.put("Act","discuss");
        msgMap.put("fromQQ",fromQQ);
        msgMap.put("fromDiscuss",fromDiscuss);
        msgMap.put("cardName",q.getNick());
        msgMap.put("msg",msg);
        requstMsg = mapSave(msgMap);
        reMsg = linkState?sendServiceMsg(requstMsg):null;
        if(reMsg!=null){
            System.out.println(reMsg);
            CQ.sendDiscussMsg(fromQQ,reMsg);
        }
